...
def predict_user(user_id, user_lat=0.0, user_lng=0.0, user_phone=None):
    ...
    for obj in objects:
        try:
            ...
            # 🔔 Envoi WhatsApp si risque 2 ou 3 et distance ≤ 30
            if not notification_sent and user_phone and int(risque) in [2, 3] and distance <= 30:
                message = f"⚠️ Alerte ! Vous êtes à {round(distance,1)}m de {obj['type_infra']}  avec un risque niveau {risque}."
                send_whatsapp_alert(user_phone, message)
    ...
    risques = [r for r in results if r['risque'] in [2, 3] and r['distance'] <= 30]
    if risques:
        response = sorted(risques, key=lambda x: -x['poids'])

        
        print(f"{len(risques)} alertes à insérer pour user {user_id}")
    else:
        response = [min(results, key=lambda x: x['distance'])]
